...
if ($method === 'GET' && preg_match('#^productos/(.+)$#', $path, $m)) {
  ...
  $stmt = db()->prepare("SELECT p.*, COALESCE((SELECT SUM(CASE WHEN tipo='INGRESO' THEN cantidad ELSE -cantidad END) FROM movimientos WHERE codigo_producto = p.codigo), 0) AS stock_actual FROM productos p WHERE p.codigo = ?");
  $stmt->execute([$codigo]);
  $r = $stmt->fetch();
